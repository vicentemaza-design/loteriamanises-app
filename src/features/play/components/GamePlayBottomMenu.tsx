...
export function GamePlayBottomMenu({
  ...
  return (
    <div
      className={cn(
        'fixed bottom-0 left-0 right-0 z-50 border-t border-white/5 bg-manises-blue/88 pt-6 shadow-[0_-8px_32px_rgba(0,0,0,0.25)] backdrop-blur-3xl',
        className
      )}
      style={{ paddingBottom: 'env(safe-area-inset-bottom, 0px)' }}
    >
      <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-white/10 to-transparent" />
      <div className="-translate-y-[16px] mx-auto grid min-h-14 w-full max-w-screen-sm grid-cols-[minmax(min-content,1fr)_minmax(min-content,1fr)_minmax(5.5rem,2.15fr)] text-white">
