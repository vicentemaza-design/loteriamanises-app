...
interface NinoPlayPageProps { game: LotteryGame }

export function NinoPlayPage({ game }: NinoPlayPageProps) {
  ...
  const { items: showcaseItems } = useNationalShowcase('nino');
  ...
  const handleManualConfirm = (items: Array<{ number: string; quantity: number }>) => {
    const drawDates = [game.nextDraw];
    const allDrafts: PlayDraft[] = [];
